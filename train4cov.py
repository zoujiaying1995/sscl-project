...
class Instructor:
    def __init__(self, opt):
        ...
        val_len = int(len(stance_dataset.train_data) * opt.val_ratio)
        train_len = int(len(stance_dataset.train_data) - val_len)
        train_set, val_set = torch.utils.data.random_split(stance_dataset.train_data, [train_len, val_len])
        self.train_data_loader = DataLoader(dataset=train_set, batch_size=opt.batch_size, shuffle=True)
        self.val_data_loader = DataLoader(dataset=val_set, batch_size=opt.batch_size, shuffle=True)
    ...
    def _train(self, criterion, unitedCL_criterion, optimizer):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                united_labels = sample_batched[self.opt.col_map['united_labels']].to(self.opt.device)
                features, outputs, fc_input = self.model(inputs)
                cl_loss = torch.tensor(0)
                cl_sentiment = unitedCL_criterion(features, senti_labels)
                # cl_stance = stance_CL_criterion(features,senti_labels,targets)
            ...
            val_acc, val_f1, val_f1_m, val_f1_0, val_f1_1, val_f1_2, val_f1_all = self._evaluate_acc_f1(
                self.val_data_loader)
            print(
                'val_acc: {:.4f}, val_f1: {:.4f}, val_f1_m: {:.4f}, val_f1_0: {:.4f}, val_f1_1: {:.4f}, val_f1_2: {:.4f},val_f1_all: {:.4f}'.format(
                    val_acc, val_f1, val_f1_m, val_f1_0, val_f1_1, val_f1_2, val_f1_all))
    ...
    def run(self, repeats=1):
        ...
        for i in range(repeats):
            ...
            # optimizer = self.opt.optimizer(self.model.parameters(), lr=0.00002, weight_decay=1e-5)  # Todo
            ...
            test_acc, test_f1, test_f1_m, test_f1_0, test_f1_1, test_f1_2, test_f1_all = self._evaluate_acc_f1(
                self.test_data_loader)
            print(
                'test_acc: {:.4f}, test_f1: {:.4f}, test_f1_m: {:.4f}, test_f1_0: {:.4f}, test_f1_1: {:.4f}, test_f1_2: {:.4f},test_f1_all: {:.4f}'.format(
                    test_acc, test_f1, test_f1_m, test_f1_0, test_f1_1, test_f1_2, test_f1_all))
            # -------------记录日志 start-------------------------------#
            log_ = {}
            for arg in vars(self.opt):
            ...
            log_data = {'timestamp': opt.logger.getTimestr(time.time()),
                        ...
                        'max_test_f1_avg': test_f1_m, 'test_f1_0': test_f1_0, 'test_f1_1': test_f1_1,
                        'test_f1_2': test_f1_2, 'test_f1_all': test_f1_all}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='mul_sh', type=str, help='mul_sh,mul_sc,mul_wa,mul_af')
    ...
    parser.add_argument('--seed', default=None, type=int)  # 766
    parser.add_argument('--device', default=None, type=str)
    parser.add_argument('--bert_path', default='bert-base-uncased', type=str)
    parser.add_argument('--temperature', default=0.05, type=float)  # sentiment_cl
    parser.add_argument('--temperatureP', default=0.14, type=float)  # stance_cl
    parser.add_argument('--cl_alpha', default=0.5, type=float)  # sentiment_cl
    parser.add_argument('--cl_beta', default=0.5, type=float)  # stance_cl
    parser.add_argument('--gama', default=1, type=float)  # class_loss_weight
    ...
    need_fields = {'model_class', 'num_epoch', 'temperature', 'temperatureP', 'cl_alpha', 'cl_beta', 'maxlen',
                   'dataset', 'isCL',
                   'mid_layer', 'timestamp', 'max_test_acc_avg', 'model_intro',
                   'max_test_acc', 'max_test_f1_ma_mi_avg',
                   'max_test_f1_avg', 'test_f1_0', 'test_f1_1', 'test_f1_2', 'test_f1_all'
                   }
    ...
    log_columns = ['timestamp', 'max_test_acc_avg', 'max_test_f1_avg', 'max_test_f1_m_avg',
                   'opt_args']
    opt.log_path = './toad_log/' + opt.dataset + '_sentiDCL' + '.csv'
